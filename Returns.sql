...
SELECT
	...
	(CASE WHEN WIN_LOSE = 1 THEN (BSP - 1.0) * .02 ELSE 0 END) AS backCommission,
	(CASE WHEN WIN_LOSE = 1 THEN (BSP - 1.0) - ((BFSP - 1.0) * .02) ELSE -1.0 END) AS netReturn,
	...
	IIF(WIN_LOSE = 1, 0.00, 0.20) AS layCommission,
	IIF(WIN_LOSE = 1, (1.0 * (BFSP-1.0) * -1.0), 0.98) AS netLayReturn
